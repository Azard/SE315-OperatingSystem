...
static struct Page *chunk_list;  // useless
static struct Page chunck_list;  // real used
...
mem_init(void)
{
	...
	// Your code goes here:
	boot_map_region(kern_pgdir, UPAGES, ROUNDUP(npages*page_size, PGSIZE), PADDR(pages), PTE_P | PTE_U);
...
page_alloc_npages(int alloc_flags, int n)
{
	...
	while(1) {
		...
		for (i = 0; i < n; i++) {
			...
			if ((page2pa(t_find) - page2pa(t_find->pp_link)) != PGSIZE)
				break;
