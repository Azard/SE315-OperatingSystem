...
spin_lock(struct spinlock *lk)
{
	...
	//cprintf("CPU %d: get ticket %d, now own %d\n", cpunum(), get_ticket, lk->own);
	while(1) {
		//cprintf("CPU %d: try ticket %d, now own %d\n", cpunum(), get_ticket, lk->own);
		if (get_ticket == lk->own) {
			//cprintf("CPU %d: lock %d, now own %d\n", cpunum(), get_ticket, lk->own);
			break;
		}
	}
...
spin_unlock(struct spinlock *lk)
{
	...
	//cprintf("CPU %d: unlock now own %d\n", cpunum(), lk->own);
#endif
}
